...
int main(int argc, char *argv[]) {
  ...
  cout << "The number of phonon pseudosite (total) =" << (Lx*Ly-Ly)*Np <<endl;
  ...
  for(size_t i=0;i<N;++i){
    ...
    if(residue<Ly) {
      ...
      if(residue!=(Ly-1)) Ty[i] = i+1;
      else Ty[i] = i - (Ly-1);
    } else {
      residue = residue - Ly;
      size_t residue2 = residue%Np;
  ...
  for (size_t i = 0; i < N; i++){
    ...
    if(Tx[i]!=-1) { //it is a electron site
      size_t y=i%total_Ly;
      size_t x_coor = i/total_Ly;
      int phonon1 = x_coor*total_Ly+ Ly + Np*y;
