...
x_coor = zeros(1, size(ChargeDensityData,1));
y_coor = zeros(1, size(ChargeDensityData,1));
total_Ly = (2*Np+1)*Ly - Np;
x = 0;
for i=1:numel(x_coor)
    y_coor(i) = x;
    global_site_num = ChargeDensityData(i, 1);
    x_coor(i) = fix(global_site_num/total_Ly);
    x = x+1;
    if x == 4
        x = 0;
    end
...
ChargeDensity = zeros( numel(Dset), numel(x_coor) );
...
% plot(y_coor + 1, ChargeDensity,'-x'); hold on;
% imagesc(x_coor, y_coor, ChargeDensity);hold on;
ChargeDensity = reshape(ChargeDensity, 4,[]);
imagesc(1:Lx, 1:Ly, ChargeDensity);hold on;
colorbar
set(gca,'YDir','normal');
