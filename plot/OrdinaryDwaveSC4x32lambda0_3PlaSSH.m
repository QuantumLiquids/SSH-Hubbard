...
A = jsondecode(fileread(['../data/scsPSaPLA',FileNamePostfix]));
distance=zeros(1,numel(A)/4);
for i=1:numel(A)/4
    distance(i) = (A{i}{1}(3)-A{i}{1}(1))/(4+2*Np);
end
...
for j = 1:numel(Dset)
    ...
    FileNamePostfix=['ssh',num2str(Ly),'x',num2str(Lx),'U',num2str(U),'g',num2str(g),'omega',num2str(omega),'Np',num2str(Np),'hole',num2str(Numhole),'D',num2str(D),'.json'];
    A = jsondecode(fileread(['../data/scsPSaPLA',FileNamePostfix]));
    B = jsondecode(fileread(['../data/scsPSbPLA',FileNamePostfix]));
    C = jsondecode(fileread(['../data/scsPScPLA',FileNamePostfix]));
    D = jsondecode(fileread(['../data/scsPSdPLA',FileNamePostfix]));
    for i=1:numel(A)
...
%     scsyx_ex(i)=p.p3;
% end
h1=loglog(distance, -(scsyx),'-^');hold on;
%%===== P_yy =======%%
