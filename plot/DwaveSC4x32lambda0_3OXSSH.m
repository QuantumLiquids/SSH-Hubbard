...
A = jsondecode(fileread(['../data/scsPSaOX',FileNamePostfix]));
...
for j = 1:numel(Dset)
    ...
    FileNamePostfix=['ssh',num2str(Ly),'x',num2str(Lx),'U',num2str(U),'g',num2str(g),'omega',num2str(omega),'Np',num2str(Np),'hole',num2str(Numhole),'D',num2str(D),'.json'];
    A = jsondecode(fileread(['../data/scsPSaOX',FileNamePostfix]));
    B = jsondecode(fileread(['../data/scsPSbOX',FileNamePostfix]));
    C = jsondecode(fileread(['../data/scsPScOX',FileNamePostfix]));
    D = jsondecode(fileread(['../data/scsPSdOX',FileNamePostfix]));
    for i=1:numel(A)
