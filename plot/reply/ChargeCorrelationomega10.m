...
FileNamePostfix=['ssh',num2str(Ly),'x',num2str(Lx),'U',num2str(U),'g',num2str(g,8),'omega',num2str(omega),'Np',num2str(Np),'hole',num2str(Numhole),'D',num2str(Db),'.json'];
...
% ReducedChargeCorrelation_ex = ReducedChargeCorrelation(end,:);
...
T=text(7,4.5e-3,['$K_{c}=',num2str(-p.p1, 2),'$']);
set(T,'Interpreter','latex');set(T,'Fontsize',24);
...
ylabel('$|\langle n(r)n(0)\rangle -\langle n(r)\rangle \langle n(0)\rangle|$','Interpreter','latex');
set(get(gca,'XLabel'),'FontSize',24);
set(get(gca,'YLabel'),'FontSize',24);


figure_directory = '../../figure';
figure_name_eps = 'ChargeCorrelationomega10.eps';
figure_path = fullfile(figure_directory, figure_name_eps);
saveas(gcf, figure_path, 'epsc');
disp(['the charge correlation figure has been saved at ', figure_path]);
